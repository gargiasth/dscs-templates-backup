    ...
    for hit in hits:
        ...
        for key, val in hit.items():
            if isinstance(val, list):
                ...
                row[key] = val
        rows.append(row)
    logger.info(f"extract_default — {len(rows)} rows")
    return rows
...
def extract_cases(hits: list[dict]) -> list[dict]:
    ...
    for hit in hits:
        ...
        rows.append(row)
    logger.info(f"extract_cases — {len(rows)} rows")
    return rows
...
def extract_samples(hits: list[dict]) -> list[dict]:
    sample_mappings = [
        ...
        if api_path.startswith("samples.")
    ]
    rows = []
    for hit in hits:
        ...
        for sample in hit.get("samples", []):
            ...
            rows.append(row)
    logger.info(f"extract_samples — {len(rows)} rows")
    return rows
...
def extract_slides(hits: list[dict]) -> list[dict]:
    slide_mappings = [
        ...
        if api_path.startswith("samples.portions.slides.")
    ]
    rows = []
    for hit in hits:
        case_id = hit.get("case_id")
        for sample in hit.get("samples", []):
            ...
            for portion in sample.get("portions", []):
                for slide in portion.get("slides", []):
                    ...
                    rows.append(row)
    logger.info(f"extract_slides — {len(rows)} rows")
    return rows