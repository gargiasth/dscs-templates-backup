...
from config.compute import setup_compute
...
def run_bronze_landing() -> pd.DataFrame:
    ...
    logger.info(f"bronze_landing — {len(df)} rows, {len(df.columns)} columns")
    return df
...
def run_bronze_request_api() -> list[dict]:
    hits = fetch_cases()
    logger.info(f"Fetched {len(hits)} cases from GDC API")
    return hits
...
def run_bronze_cases(hits: list[dict]) -> pd.DataFrame:
    ...
    logger.info(f"bronze_cases — {len(df)} rows")
    return df
...
def run_bronze_samples(hits: list[dict]) -> pd.DataFrame:
    ...
    logger.info(f"bronze_samples — {len(df)} rows")
    return df
...
def run_bronze_slides(hits: list[dict]) -> pd.DataFrame:
    ...
    logger.info(f"bronze_slides — {len(df)} rows")
    return df
...
def run_silver_cases() -> pd.DataFrame:
    ...
    logger.info(f"silver_cases — {df.shape}")
    return df
...
def run_silver_samples() -> pd.DataFrame:
    ...
    logger.info(f"silver_samples — {df.shape}")
    return df
...
def run_gold_mastertable() -> pd.DataFrame:
    ...
    logger.info(f"gold_mastertable — {df.shape}")
    return df